...
// QueryTagg chains the current query on the "tagg" edge.
func (tq *TaskQuery) QueryTagg() *TagQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, task.TaggTable, task.TaggColumn),
		)
		fromU = sqlgraph.SetNeighbors(tq.driver.Dialect(), step)
		return fromU, nil
	}
...
// WithTagg tells the query-builder to eager-load the nodes that are connected to
// the "tagg" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithTagg(opts ...func(*TagQuery)) *TaskQuery {
	query := &TagQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TaskQuery) sqlAll(ctx context.Context) ([]*Task, error) {
	...
	if query := tq.withTagg; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Task)
		for i := range nodes {
			fk := nodes[i].TagID
