...
func TagID(v int) predicate.Task {
	return predicate.Task(func(s *sql.Selector) {
...
func TagIDEQ(v int) predicate.Task {
	return predicate.Task(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldTagID), v))
	})
}
...
func TagIDNEQ(v int) predicate.Task {
	return predicate.Task(func(s *sql.Selector) {
...
func TagIDIn(vs ...int) predicate.Task {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
...
func TagIDNotIn(vs ...int) predicate.Task {
	v := make([]interface{}, len(vs))
	for i := range v {
		v[i] = vs[i]
	}
...
func TagIDIsNil() predicate.Task {
	return predicate.Task(func(s *sql.Selector) {
...
func TagIDNotNil() predicate.Task {
	return predicate.Task(func(s *sql.Selector) {
...
// HasTagg applies the HasEdge predicate on the "tagg" edge.
func HasTagg() predicate.Task {
	return predicate.Task(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(TaggTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, TaggTable, TaggColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasTaggWith applies the HasEdge predicate on the "tagg" edge with a given conditions (other predicates).
func HasTaggWith(preds ...predicate.Tag) predicate.Task {
	return predicate.Task(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(TaggInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, TaggTable, TaggColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
