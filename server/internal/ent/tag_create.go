...
func (tc *TagCreate) SetMult(f float64) *TagCreate {
	tc.mutation.SetMult(f)
	return tc
}
...
func (tc *TagCreate) check() error {
	...
	if _, ok := tc.mutation.Mult(); !ok {
		return &ValidationError{Name: "mult", err: errors.New(`ent: missing required field "mult"`)}
	}
	if _, ok := tc.mutation.UserID(); !ok {
...
func (tc *TagCreate) createSpec() (*Tag, *sqlgraph.CreateSpec) {
	...
	if value, ok := tc.mutation.Mult(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
