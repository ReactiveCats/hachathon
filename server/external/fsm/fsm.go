...
func NewTransition(handler HandlerFunc, text TextFunc, keyboard KeyboardFunc, opt ...interface{}) Transition {
	obj, err := ShouldNewTransition(handler, text, keyboard, opt...)
	if err != nil {
		panic(err)
	}
	return obj
...
func ShouldNewTransition(handler HandlerFunc, text TextFunc, keyboard KeyboardFunc, opts ...interface{}) (Transition, error) {
	if handler == nil {
	...
	if !transition.GoForward && !transition.GoForwardSilent {
		...
		if keyboard == nil {
			return Transition{}, errors.New("keyboard func is nil")
		}
	}
...
func (fsm BotFSM) Handle(vars map[string]interface{}, state string, arguments []string, event string) (string, []string, error) {
	...
	if nextMenu.Transition.GoForward {
		return fsm.Handle(vars, nextState, newArguments, WildcardToken)
	}
...
//Validate will check fsm for transitions to state, that does not exist
